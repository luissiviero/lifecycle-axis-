...
- [The tap in the Actions tab is the human act](approve-by-dispatch.md) — amends human-only-approvals.md and delegated-mode.md: a workflow_dispatch run writes what the approval script writes, with the run's actor as the deciding handle; the chain check and the merge script resolve the commit's Approved-Run trailer against the run record
- [The merge advances the pointer; the session only follows it](run-queue.md) — amends delegated-mode.md decision 6: after merging, the workflow moves .sdlc/active to the next granted, unstarted item and writes a ledger line on both, behind a staged-path allowlist and a lost-update guard; an item's own pull request never can, because .sdlc is on the merge script's ALWAYS_LOCKED floor
- [A delegated run detours around non-low work by a route record, and parks with two ledger words instead of stopping the queue](risk-detour.md) — amends delegated-mode.md decision 4: scripts/check_detour.py calls the merge script's own locked-path matcher and ends DETOUR: none or needed; a revision record of kind detour carries the route reviewers judge; a parked: ledger line on intent.md is what next_item.py skips and gen_index.py marks, and the park lands as the item's own delegated pull request so the advance moves the pointer
- [What the CI budget loosened, and what stayed crucial](ci-budget-crucial-and-loosened.md) — six controls that did not move, each naming the file that proves it, against six that were loosened with their compensating control; a skipped required check counts as passing, which is the fact to weigh before creating a branch-protection rule
