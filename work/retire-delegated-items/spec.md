...
| R-7 | **The role gate covers every artifact the retirement will write, on the default branch only.** `approve_dispatch.check_actor(login, artifact, mode="retire", root=, slug=, ref=, default_branch=)` requires `slug`, ignores the `artifact` input's value, requires `login` to hold the role of every chain artifact present under `work/<slug>/`, and requires `ref` to be the default branch as a grant does (a retirement moves the pointer, and a work branch whose diff moves the pointer is strict for the chain check; security pass on #92); a slug with no artifacts is refused. Supervised and delegated behaviour is unchanged. | 2 and 3 | `scripts/test_approve_dispatch.py::Retire::test_retire_requires_every_present_artifacts_role` (an approvers file where the actor holds `product-owner` but not `tech-lead`: refused naming `plan.md`); `test_retire_with_every_role_passes`; `test_retire_without_a_slug_is_refused`; `Mode` cases green unmodified. Red today (`mode="retire"` passes as if supervised). |
...
- `scripts/approve.py <slug> [artifact ...] --retire [--next SLUG] [--as HANDLE] [--note TEXT] [--from-dispatch RUN_ID] [--dry-run]`.
  ...
  may not name the item being retired`, `approve: --next applies to --retire only`, `approve: --next moves the
  pointer off an item that is still live (<artifacts> not retired); retire every artifact, or omit --next`
  (a partial retirement never clears the pointer either: `pointer: .sdlc/active still names the item, which
  is not fully retired; left as it is`; both from the security pass on #92). A slug is judged by
  `check_artifact_chain.SLUG_RE`, already the
...
- `scripts/approve_dispatch.py --check-actor LOGIN --artifact NAME --mode retire --slug S [--root DIR]`:
  `ok` / exit 0, or exit 1 with `approve-dispatch: LOGIN may not approve <artifact>: <name>: <reason>` for
  the first artifact refused, `... work/S has no chain artifact to retire`, or `... a retirement must be
  dispatched from the default branch (<name>), not '<ref>'`.
...
`delegated -> superseded` with `approved-by: claude` and whose ledger carries `in-review -> delegated | claude`
for each (the line the binding of revision 1 reads, and the actor is the handle in `approved-by`), goes from
red to green, which is R-1 in production. No file under `work/` other than this item's is rewritten by this
item.

### Failure modes and how they surface
- Tap with `retire` and a blank slug: the Resolve step exits 1 with `::error::a retirement must name its slug
  explicitly`; nothing written. Tap with `retire` from any branch but the default: the role gate refuses
  naming the ref; nothing written.
- `--retire` naming some artifacts of a live item: they are superseded, the pointer is left as it is with a
  note, and `--next` is refused naming the artifacts still live.
...
- C3: R-1 removes one check from the chain (the approver list on a `superseded` artifact the ledger shows
  signed under a grant; revision 1); the weight moves to the ledger
...
- D1: **`superseded` is judged by the retirer, never by `approved-by`.** Context: `approved-by` on a retired
  ...
  for the artifact (no `-> delegated` line whose actor is that handle), because a human-approved artifact's
  approver was a valid handle and a bot handle there is a malformed record (the pre-existing case pins it;
  amended under plan deviation 3 and revision 1; the actor binding is the security pass's, so a stray
  `-> delegated` line by another handle switches nothing off). One more consequence: a retired signed
  artifact is valid only while that `-> delegated` line is in its ledger, so the ledger's append-only rule
  is load-bearing for retirement validity; a truncated ledger fails closed with a human fix (revision 1). Consequences:
