...
---
# A fresh install of the kit breaks in the first hour; the adopter path must work end to end

- [intent.md](intent.md) — status: approved; approved-by: luissiviero; The install omits the approval script, ships the owner's handle in every role, silently skips an existing settings file, reports a false VERIFY PASS on a placeholder, leaves the plan gate open, and gives the adopter no GitHub-side checklist.
