...
- .claude/skills/** — the five stage skills and security-standards; their text quotes the template spellings (deviation 2026-09-04)
- .claude/agents/** — the four read-only subagents; same (deviation 2026-09-04)
...
- 2026-09-04: local suite time on the owner's Windows PC, measured: 280 tests in 592 s, 332 s of it in `scripts/test_adopt.py`, which ran the full `adopt.sh` 22 times. Now one shared target per class plus six purpose-built scenarios, built concurrently (19 tests, same assertions, 370 s -> 84 s); `scripts/run_tests.py` runs test modules concurrently with the serial runner's discovery, exit code and summary shape (`-j 1` restores serial), and `VERIFY_CMDS` calls it. Found on the way: `gen_index.py --check` compared bytes, so a `core.autocrlf` checkout reported `work/index.md` drifted until someone regenerated; it now normalises CRLF like `gen_context_files.py --check` already did. Full `verify.sh` here: ~780 s -> 242 s; Linux CI 11 s. Files: `.sdlc/config.env`, `scripts/run_tests.py`, `scripts/test_run_tests.py`, `scripts/test_adopt.py`, `scripts/gen_index.py`, `scripts/test_gen_index.py`, `docs/sdlc/README.md`.
- 2026-09-04: steps 1 and 2 of retiring this umbrella plan, the last two changes it carries. (1) `check_artifact_chain.py` gains an in-progress mode: when the PR diff touches only `work/<slug>/` and the generated `work/index.md` (or is empty), the chain is checked as far as it exists; a PR that touches another item's artifacts under this slug is mislabelled and gets the strict check -- `intent.md` present, each later artifact only once the previous is approved, every `status` in the enum, `log.md` present, approver and ledger checks unchanged for anything approved -- so a work item can be opened and approved one stage per PR, which PR #17 showed was impossible (its intent-only PR could never be green). A diff outside `work/` still needs the whole chain approved. `sdlc-gate.yml` is unchanged. Tests: `InProgressChain` (7) in `scripts/test_check_artifact_chain.py`; eval: `chain-allows-intent-only-pr`. (2) `adopt.sh` rewrites `PLAN_REQUIRED_PATHS` in a target to the application default (`src lib app services packages`) the way it already rewrites `VERIFY_CMDS`, so the kit's own value can name the kit's product code (`scripts`, the owner's step 3) without leaking into adopters. Test in `scripts/test_adopt.py`. Files: `scripts/check_artifact_chain.py`, `scripts/test_check_artifact_chain.py`, `evals/cases/chain-allows-intent-only-pr.yaml`, `scripts/adopt.sh`, `scripts/test_adopt.py`, `docs/sdlc/README.md`.
- 2026-09-04: two gaps in the in-progress chain mode found while writing the owner's instructions for retiring this item: a `superseded` artifact keeps the `approved-by` it earned (the check flagged it as "approved-by without approved"), and `.sdlc/active` lives outside `work/`, so the commit that activates an item -- or retires this one and activates the next -- always fell into the strict check. `.sdlc/active` now counts as the item's own file when the diff points it at the slug being checked; pointing it anywhere else stays strict. Three tests in `InProgressChain`. Files: `scripts/check_artifact_chain.py`, `scripts/test_check_artifact_chain.py`.
- 2026-09-04: CI review of PR #20 (security, Important): `superseded` was exempt from the approver, ledger and commit-author checks, so `status: superseded` with `approved-by: claude[bot]` would have passed. Superseding now gets the same three checks as approving: `approved-by` must be a valid approver, `log.md` must carry an `approved -> superseded` line whose actor holds the artifact's role (need not be the original approver), and the commit that set `status: superseded` must not be agent-authored. Two more tests in `InProgressChain`.
- 2026-09-04: PR #21 (`PLAN_REQUIRED_PATHS="scripts"`) turned six hook tests and one eval red: `hooktest.fake_repo()` copied this repo's live `.sdlc/config.env`, so every test that wrote into `src/` expecting the plan gate was coupled to the kit's own governance value. The fake repo is an adopter's repo and now gets the adopter default for that one key (the same rewrite `adopt.sh` makes); `hook-requires-approved-plan` writes under whichever path the repo it runs in actually gates. Files: `scripts/hooktest.py`, `evals/cases/hook-requires-approved-plan.yaml`.
- 2026-09-04: the stage skills had drifted from the templates they fill: `/sdlc-spec` step 2 said `standards-applied` where `docs/sdlc/templates/spec.md` has `skills-applied` (and `work/_example/spec.md` shipped that spelling), and `/sdlc-plan` steps 2 and 4 plus both `plan-reviewer` agents said `## Files` and `## Verification` where `templates/plan.md` has `## Files that change` and `## Proof`. The chain check matches headings by prefix, so the file list still worked; the proof mapping and the provenance field did not. Skills and agents now quote the templates, `work/_example/spec.md` is renamed, eval `skill-names-match-templates` pins the spellings, and the lesson is in CLAUDE.md (rule 7). `.claude/skills/**` and `.claude/agents/**` were missing from this file list and are added. Files: `.claude/skills/sdlc-spec/SKILL.md`, `.claude/skills/sdlc-plan/SKILL.md`, `.claude/agents/plan-reviewer.md`, `.gemini/agents/plan-reviewer.md`, `work/_example/spec.md`, `evals/cases/skill-names-match-templates.yaml`, `CLAUDE.md`, `work/sdlc-kit-phase-1/plan.md`.
