...
| R-8 | **The loop is green.** `scripts/verify.sh` ends `VERIFY: PASS`, `python3 scripts/check_artifact_chain.py --base origin/main --slug advance-push` ends `CHAIN: PASS`, `scripts/run_evals.sh` ends `0 fail`, `python3 scripts/check_okf.py` ends `0 warnings`, both generators report up to date. | 4 | The last lines, pasted in the pull request; the new module's six cases counted, four of them seen red before the code. |
