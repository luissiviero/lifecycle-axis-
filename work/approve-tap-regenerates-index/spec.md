...
- `event_default_branch(path=None) -> str | None`: reads `path` or `GITHUB_EVENT_PATH`; returns
  ...
  malformed file (R-11).
- `route(ref=None, default_branch=None) -> bool`: `ref` falls back to `GITHUB_REF_NAME`, `default_branch`
  to `event_default_branch()`; `False` unless `is_default_branch` holds and `GITHUB_REF_TYPE` is unset
  or `branch`.
...
- `changed_paths(root=None) -> (judged, staged)`: two sorted lists from one `git status --porcelain
  --untracked-files=all`; a rename or copy entry contributes both ends to `judged` and its destination to
  `staged`; every other entry contributes its path to both.
...
- **The route cannot be decided.** Every malformed or missing input to R-11 yields `None` and the narrow
  route; nothing raises. The tap then behaves as before this item for foreign indexes (leaves them) and
