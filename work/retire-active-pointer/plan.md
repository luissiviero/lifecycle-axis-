...
- work/retire-active-pointer/spec.md — signed at the design stage before this plan; revised under revisions/1.md after the review (deviation 2)
- work/retire-active-pointer/revisions/1.md — the consensus record for that revision (deviation 2)
- docs/sdlc/README.md — the one-line inventory of what check_artifact_chain.py checks, brought up to date (deviation 2)
...
6. **R-6, the loop.** `GH_TOKEN= GITHUB_TOKEN= scripts/verify.sh` and `GH_TOKEN= GITHUB_TOKEN= python3 scripts/check_artifact_chain.py --base origin/main --slug retire-active-pointer` (both unset the tokens: `verify.sh` runs the chain check too, and risk 4's crash hits either one where `gh` is absent); `scripts/run_evals.sh`; `python3 scripts/check_okf.py`. Regenerate indexes. Paste the four last lines and the test counts before and after in the pull request.
...
- 2026-09-08, step 3: "retired" is judged on the **base ref**, not the head. The pre-existing case `InProgressChain.test_fully_superseded_chain_passes` (work/batch-b-followups R-3) models the pull request that retires an item while the pointer still names it; that is the act in progress and must pass. So the R-2 error fires when `work/<active>/intent.md` is already `superseded` on `--base`, and the head's `superseded` intent earns a note telling the human to move the pointer in the same pull request. R-2's oracle is unchanged (`--base HEAD` still fails); a fourth case, `test_retiring_pull_request_is_a_note_not_a_failure`, pins the other side. No file is added to the list. Under risk 1 in the same commit, not a deviation: the two-line rules addition put the adopter's `CLAUDE.md` at 121 of 120, so the fragment's paragraph was re-flowed to net zero lines, prose only, every rule kept.
- 2026-09-08, review round 1 (security-reviewer and plan-reviewer, both on opus; writer Fable 5.1): three Important findings, each fixed with a regression test. (1) `--base HEAD`, the self-check `scripts/verify.sh` runs, has no "before", so the previous entry's "still fails" was wrong there: a retiring pull request passed CI's chain step and failed its Verify step on the same tree. Now a retired pointer under a self-check is a note, and the failure is CI's, where the base is `origin/main`; R-2's case moved to a real base. (2) The pointer reached `git show` and `os.path.join` unvalidated (`../x` read outside the repo, `./demo` bypassed R-2, a NUL crashed with no `CHAIN:` line, a newline could forge one): `SLUG_RE` at the boundary, one FAIL line, `!r` in the message; the lesson `one-path-spelling-in-guards.md` recurring, applied. (3) The signed spec's R-2 row and "exact shapes" no longer described the code: revised under `revisions/1.md` with both reviewers' `verdict: revise`, re-signed with `--revision`. Nits taken: `git diff`'s stderr on the R-4 line; a pointer to an item on neither base nor head is a note; R-3's test counts every note; step 6 unsets the tokens for `verify.sh` too, for the risk-4 crash; `docs/sdlc/README.md`'s inventory line updated. Three files added to the list: `spec.md` (added at the design stage and missing from the list), `revisions/1.md`, `docs/sdlc/README.md`.
- 
