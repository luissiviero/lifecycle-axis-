...
- 2026-09-05 — step 5, from the security pass on pull request 45 (seven Important findings, all fixed in the same pull request; three narrow R-15's words, all on the closed side). (1) `verification.verified` vouches for the committer's signature while `author.login` comes from an author email the committer sets freely, so a signed commit with a spoofed owner author passed: the committer must now hold `product-owner` too, or be `web-flow` (GitHub's own signer for web-editor and API commits), and `verification.reason` must be `valid`. (2) The grant was read from the head, which the pull request itself writes: the intent and its grant commit are now read from the base branch, and a diff touching `work/<slug>/intent.md` is refused (R-15 said "the head's `intent.md`"; the base's is the only copy the judged diff cannot have written). (3) Any `claude[bot]` comment satisfied `require-review`, including one the writing session posts with `gh pr comment`: the verdict is now read only from the tracking comment that links a successful `pr-review` run of this head sha, the earliest such comment, which the action opens when its run starts. (4) The freshness check compared pusher-set commit dates; binding the comment to the run's head sha replaces it. (5) A required workflow's run was matched by name only, so a manual `workflow_dispatch` run could stand in for the `pull_request` one: runs are filtered to `event: pull_request` on the head branch and all of them must be green. (6) `check_locked_paths` ignored `previous_filename`, so a rename out of a locked path passed, and the reviewer's own inputs were unlocked: the script carries a floor of always-locked prefixes (`.github`, `.claude`, `.gemini`, `docs/sdlc/rules`, `docs/sdlc/templates`, the three context files, `REVIEW.md`, `.sdlc`, the verify loop) beneath the policy's list. (7) The slug came from the pull request body alone: it must now equal `.sdlc/active` on the base branch, and a `.` segment is refused. Nits: the grant window is a real slice of 100; the self check-run name is the job name `merge`; the merge comment prints the normalised handle; the event must have concluded `success`; `--head-sha` is validated; the merge title carries no head label.
- 
