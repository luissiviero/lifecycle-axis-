...
- Spec rows → tests: R1, R2 → `test_check_workflow_permissions.SdlcGate`; R3 → `PrReviewWorkflow`; R4 → `test_run_evals.AgentEvalsWorkflow`; R5 → `DelegatedMergeWorkflow`; R6 → `test_delegated_merge.Plumbing`, `ChecksCondition`, `ReviewCondition`, `CoolOffCondition`; R7 → `DelegationCondition`, `Plumbing`, `EndToEnd`; R8 → `skill-names-match-templates` and `skill-opens-drafts`; R9 → the adopter render at 120 lines, `context-drift.sh` and `scripts/test_adopt.py`; R10 → `ActionsMinutesSeries`, the fixture series `5.0, 2.0`, `bands_config.py` printing 3 rows; R11 → `check_okf.py` and the index line; R12 → the two `git diff` assertions; R13 → the two `grep` content checks of step 5, then `check_okf.py` and `verify.sh`; R14 → the acceptance numbers in the closing ledger line
