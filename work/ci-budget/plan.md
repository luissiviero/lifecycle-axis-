...
milestone revision runs under the existing `plan-reviewer` and `security-reviewer` roles pinned to Fable
rather than by switching the session model. Using the named roles, not an ad-hoc subagent, is what keeps
rule 8 satisfied: those files carry the bounded `tools:` list that makes a subagent read-only, so there is
still one writer per item (`knowledge/decisions/one-writer-until-ledger.md`), and the model is a pin on the
role as `docs/sdlc/spikes/prompt-surfaces.md` prescribes.
...
- .github/workflows/pr-review.yml — R3 concurrency, the `local` head-repository guard, which gates the checkout step's own `if:` rather than only reporting beside it, the pin step extended to `.claude/`, `CLAUDE.md`, `GEMINI.md`, `AGENTS.md` on the comment route; header comment
...
- scripts/delegated_merge.py — R6 the two `_pr_runs` clauses and the same superseded rule for a `cancelled` row in `check_check_runs`, which reads a different API list where `cancelled` is not an accepted conclusion; R7 the `delegation` condition, its place in `run()`, the `keep_going` rule, `_code()` and `finish()`. No new GitHub API call, and no `permissions:` block in any of the four workflows changes, which `knowledge/lessons/workflow-permissions-name-every-api.md` asks to be said out loud
...
4. `scripts/delegated_merge.py` R6, then R7, each with its cases from the spec's third column, and three invariants the diff must satisfy rather than merely not break:
   ...
   - the same superseded rule reaches `check_check_runs`, which reads the commit's check-run rows rather than `_pr_runs` and does not accept `cancelled`; without it a head sha whose first attempt was evicted stays unmergeable, which is the deadlock R6 exists to clear, so the end-to-end case proves the whole pipeline and not the two conditions alone. This extends R6 past the three functions the spec's row names, in service of the same requirement;
   ...
   Verifiable: `python3 -m unittest scripts.test_delegated_merge` green; `git diff origin/main -- scripts/test_delegated_merge.py` shows no removed `def test_`; `scripts/test_check_artifact_chain.py` untouched.
5. R13's sentences in the six documents, `evals/README.md` and the three workflow headers. Verifiable by content, since `check_okf.py` reads front matter and links rather than prose: `grep -n "agent-evals" docs/sdlc/github-setup.md docs/sdlc/README.md evals/README.md` names it only as the nightly and never as a pull-request trigger or a required check; `grep -n "triage" docs/sdlc/github-setup.md` finds the new bullet; then `python3 scripts/check_okf.py` ends `0 warnings`.
...
- Risk: R6 clears the deadlock in the two conditions that read `_pr_runs` but not in `check_check_runs`, which reads the commit's own check-run rows and treats `cancelled` as a failure, leaving the pull request unmergeable for the same reason → mitigation: step 4 carries the rule into that function too and proves it end to end rather than per condition; the failure mode while it is missing is fail-closed, a refusal rather than a merge.
...
- Spec rows → tests: R1, R2 → `test_check_workflow_permissions.SdlcGate`; R3 → `PrReviewWorkflow`; R4 → `test_run_evals.AgentEvalsWorkflow`; R5 → `DelegatedMergeWorkflow`; R6 → `test_delegated_merge.Plumbing`, `ChecksCondition`, `ReviewCondition`, `CoolOffCondition`, and an `EndToEnd` case carrying an evicted attempt through both the workflow-run list and the check-run list; R7 → `DelegationCondition`, `Plumbing`, `EndToEnd`; R8 → `skill-names-match-templates` and `skill-opens-drafts`; R9 → the adopter render at 120 lines, `context-drift.sh` and `scripts/test_adopt.py`; R10 → `ActionsMinutesSeries`, the fixture series `5.0, 2.0`, `bands_config.py` printing 3 rows; R11 → `check_okf.py` and the index line; R12 → the two `git diff` assertions; R13 → the two `grep` content checks of step 5, then `check_okf.py` and `verify.sh`; R14 → the acceptance numbers in the closing ledger line
