...
- docs/sdlc/rules/30-conventions.md — R9 one bullet in, prose re-flowed to pay for it
- docs/sdlc/rules/60-lessons.md — rule 7, the pointer line for the lesson below
- knowledge/lessons/a-verifiable-command-fails-before-the-change.md — rule 7: this item's own plan named six commands that would have passed with the work absent, all caught by one revision round; the lesson is that a `Verifiable` line must fail before the change and pass only after, never merely name a suite or match a string the file already carries
- knowledge/lessons/index.md — the lesson's line
...
8. R9 and rule 7 together, because they share one budget: the convention bullet in `docs/sdlc/rules/30-conventions.md`, the lesson file and its `index.md` line, and the pointer line in `docs/sdlc/rules/60-lessons.md`. Two rendered lines are added, so two must be paid for by re-flowing prose in the fragments, never by dropping a rule, measured against the adopter's render before anything is committed. Verifiable: `grep -c "one agent code pull request" CLAUDE.md GEMINI.md AGENTS.md` is 1 each and `grep -c "a-verifiable-command-fails-before-the-change" CLAUDE.md GEMINI.md AGENTS.md` is 1 each, which proves the bullet exists rather than only that the cap holds; then `bash scripts/adopt.sh "$SCRATCH/adopt"`, `python3 scripts/gen_context_files.py --root "$SCRATCH/adopt"` and `wc -l "$SCRATCH/adopt/CLAUDE.md"` print 120 with no "over MAX_CONTEXT_LINES"; then `python3 scripts/gen_context_files.py` here and `scripts/checks/context-drift.sh` passes.
...
- Risk: the two new rendered lines, the convention bullet and the lesson pointer, push the adopter's rendered context file over `MAX_CONTEXT_LINES`, at which point `gen_context_files.py` writes nothing at all and the adopter gets no context files → mitigation: step 8 treats them as one budget of two lines, pays both by re-flowing prose, and measures the adopter render before the commit; the repo's own files have room (108, 117 and 97 against 120), the adopter's does not, which is the whole of `knowledge/lessons/adopter-context-file-sits-at-the-cap.md`.
...
- Step 6 said "Fable M3 after the merge-conditions dry run". The dry run cannot judge PR #71's head sha: `pick_pr` returns `None` for a closed pull request, so `run()` returns before the `checks` condition, and this container has no `gh` binary to call it with anyway. M3 therefore ran against a live `check-runs` listing of that sha instead, which is the same evidence the dry run existed to fetch. It settled what the plan's third risk left open: the red-then-green same-sha shape refuses one condition earlier than expected, at `check_required_runs`, which is in-spec; the C5 evicted-attempt residual in `check_check_runs` is real, fail-closed, and goes to the owner as a spec decision rather than being widened here, exactly as that risk prescribed.
- R10 says the bucket's minutes are "divided by the distinct head branches ≠ default in the bucket". Taken as a comparison of branch *names*, that is wrong for a fork: GitHub reports `head_branch` as a bare ref, so a pull request opened from a fork's own default branch arrives as `main` and was read as this repository's trunk — its minutes counted toward the day while its pull request divided none of them, inflating every other pull request's average. The security pass on PR-B found it and it reproduces: 10.0 where the honest answer is 5.0. A run is now identified by (head repository, head branch), so "≠ default" means "not this repository's own default branch" rather than "not a branch that shares the default branch's name"; where a run carries no repository fields the old name comparison still applies, so the fixture is unchanged. This reads the spec's intent more strictly than its letter, in the direction the spec's own concern C1 asks for, and adds three tests.
