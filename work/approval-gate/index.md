...
- [intent.md](intent.md) — status: in-review; approved-by: ; No hook covers work/, so an agent can set status approved and approved-by itself or run approve.py with CLAUDECODE stripped, and the chain check does not catch a local agent; approval is a record, not a gate.
- [spec.md](spec.md) — status: in-review; approved-by: ; A PreToolUse hook that refuses agent writes to the approval fields of chain artifacts and agent invocations of approve.py, plus an approver check in require-plan.sh.
