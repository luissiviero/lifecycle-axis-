...
- docs/sdlc/rules/30-conventions.md — one paragraph re-flowed to free a line for the new lesson pointer (deviation 2); the tap paragraph re-flowed four lines to three for the same reason (deviation 4) -- the chain header cannot be touched, adopt.sh rewrites it by exact literal match
...
- docs/sdlc/README.md — the inventory gains next_item.py; delegated-merge.yml's line says it advances the pointer (deviation 4)
- docs/sdlc/rules/40-claude-only.md — `/sdlc-run` runs every granted item, not one (deviation 4)
- docs/sdlc/rules/50-gemini-only.md — the same line for Gemini (deviation 4)
- docs/sdlc/handoff/HANDOFF.md — the merge now moves the pointer; the last tap runs first; the gh-absent workaround (deviation 4)
- knowledge/lessons/adopter-context-file-sits-at-the-cap.md — new; rule 7, met on two consecutive items (deviation 4)
- knowledge/lessons/commit-before-the-chain-check.md — new; rule 7, met twice in one session (deviation 4)
...
- 2026-09-08, after the merge (deviation 4 of 5): the documentation audit before ending the session found five statements this item made false and two mistakes made twice. `/sdlc-run`'s one-line description in `40-claude-only.md`, `50-gemini-only.md` and `README.md`'s inventory still said "grant to ready pull request"; `HANDOFF.md` still said the merge has no write-back and the pointer stays until a human moves it. The skill's printed queue was wrong about the first item: the run must start at whatever `.sdlc/active` names, and every grant tap repoints it, so the *last* tap runs first — the skill now prints the active slug, then `next_item.py --list --exclude`. Two lessons under rule 7: the adopter's `CLAUDE.md` sits at exactly the cap (hit on this item and the previous one), and `check_artifact_chain.py` audits nothing on a staged-but-uncommitted tree (hit twice this session). Their two pointer lines are paid for by deleting the blank-line pointer, whose mistake `CaseBlocksAreWhole` now makes impossible, and by re-flowing the tap paragraph in `30-conventions.md` (the chain header is matched by exact literal in `adopt.sh:493`, which a first attempt broke and `test_adopt.py` caught); the adopter renders at 120, the cap, as before. No code changed.
- 
