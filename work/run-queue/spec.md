...
| R-3 | The advance is retraceable from the ledgers alone: the merged item gets `- <ts> \| PR #<n> \| in-review -> in-review \| github-actions[bot] \| <sha> \| merged as <sha>; .sdlc/active advanced to <next>`, and the next item gets `- <ts> \| intent.md \| approved -> approved \| github-actions[bot] \| <sha> \| .sdlc/active advanced here after PR #<n> merged, under the grant by <handle> on <date>`. The `<from> -> <to>` slot holds `status` values only (`knowledge/lessons/ledger-slot-holds-status-only.md`). | 3 (the whole night readable from `work/*/log.md`) | `Advance::test_ledger_lines_parse_and_name_both_items` — both lines parse; the merged item's line names the next slug; the next item's names the pull request and the grant handle |
...
- Two workflow runs racing to advance → `delegated-merge.yml:59-62` serialises by head sha, but two
  *different* pull requests could merge close together. **The push is what makes this safe**: it is
  never forced, so any commit that reached the remote first — another advance, or a human's own
  pointer edit — rejects it, and the rejection is logged with no retry, leaving the next merge to
  advance. (Revision 1: the pointer re-read inside `advance()` was credited with this and cannot be.
  `check_pull_request` has already refused any pull request whose `Work-Item` is not the active slug,
  so through `run()` it compares the file to itself; it stays as a precondition for a direct call.)
  A checkout that is not already clean writes nothing at all, since `git add` on a dirty file would
  stage that file's other changes too — the allowlist bounds which files are committed, not what is
  inside them.
