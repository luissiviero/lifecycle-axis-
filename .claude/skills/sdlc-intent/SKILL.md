...
5. Open the intent's pull request with `gh pr create --draft` (an intent-only one may run beside the active
   item's code pull request). A draft's gate and review runs conclude `skipped` in seconds, so the wait for the
   owner costs nothing; the ready click is what buys CI, and this artifact never needs it.
