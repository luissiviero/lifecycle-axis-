...
3. Write `work/<slug>/incident.md` from the template. When the intent has `mode: delegated`, run
   `python3 scripts/sign.py <slug> incident.md` and continue; otherwise stop and ask for the tap. Ask for the tap: give the owner the three inputs to pick in the Actions tab (**Actions -> approve -> Run workflow**, `.github/workflows/approve.yml`): `slug`, `artifact`, `mode`. Naming the inputs is not approving -- the owner reads the artifact and chooses, and the run records who chose.
