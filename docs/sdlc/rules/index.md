...
The fragments below are the one rule source. `scripts/gen_context_files.py` renders them, in filename order and
filtered by each fragment's `targets`, into the block between `<!-- BEGIN GENERATED -->` and `<!-- END GENERATED -->`
