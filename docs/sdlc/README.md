...
| 4 Test | Continuous evals in CI | 20–50 real tasks (`skill-*` cases; six today) beside the deterministic gate tests; the deterministic cases run in every gated commit's `verify.sh` step, and the full suite runs nightly and on demand with `--require-claude` so an expired credential is a red run; every incident adds an eval | config-owning team |
...
scripts/check_artifact_chain.py  artifacts approved by a valid approver with a log.md entry; diff ⊆ "Files that change"; release-gated paths have an owner; a diff touching only work/ is checked as far as the chain exists (one stage per PR); before any of it, one line each for a --base git does not know, a malformed .sdlc/active, and a pointer already retired on the base (a note when the retirement is this PR's own, or --slug and the pointer disagree), and an empty diff on a tree with uncommitted work or one git status cannot read (the `--base HEAD` self-check verify.sh runs is exempt)
scripts/next_item.py             the delegated queue: which granted, unstarted item is next (earliest delegated-on, ties by slug); read by delegated_merge.py's advance and by /sdlc-run to print the queue
...
.github/workflows/delegated-merge.yml  on workflow_run completion, from main only: scripts/delegated_merge.py prints every condition and merges a delegated pull request as github-actions[bot], then advances .sdlc/active to the next queued item with a ledger line on both (work/run-queue); workflow_dispatch with a head sha is a dry run
```
...
| The human act may be one tap, and the tap is recorded | `knowledge/decisions/approve-by-dispatch.md` | `.github/workflows/approve.yml` (`workflow_dispatch`, four inputs) refuses an actor outside the artifact's role, then runs the approval script as that actor and commits with `Approved-Run`/`Approved-Actor` trailers; `check_artifact_chain.py` and `delegated_merge.py` resolve them against the run record, whose `actor.login` nothing inside the run can set; `production-gate.sh` still blocks every route an agent has to press Run | presses Run, having read the artifact |
